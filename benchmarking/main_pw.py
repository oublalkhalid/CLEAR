...
def main():
    ...
    for args.seed in args.seeds:
        ...
        args.save_dir = os.path.join(
            args.model_dir,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_nn{args.nn}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}",
        )
        ...
        heatmap_file_est = os.path.join(
            heat_path_est,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_nn{args.nn}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}_Corr_heatmap.pdf",
        )
        heatmap_file_true = os.path.join(
            heat_path_true,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_nn{args.nn}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}_Corr_heatmap.pdf",
        )
        heatmap_file_indep = os.path.join(
            heat_path_indep,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_nn{args.nn}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}_Corr_heatmap.pdf",
        )
        ...
        if not args.evaluate:
            B_ori = ut.simulate_dag(
                args.z_n, args.z_n * args.DAG_dense, args.graph_type
            )
        ...
        if args.mask_size > 1:
            ...
            masks = (
                np.tril(masks, -args.z_n - 1 + ac) + np.tril(np.triu(masks), ac - 1)
            ).tolist()
        ...
        def sample_whole_latent(size, indep=False, Mask=True, device=device):
            ...
            for k in range(len(masks)):
                ...
                if k == len(masks) - 1:
                    ...
                    z[k * mini_batch : (k + 1) * mini_batch, :] = (
                        z[k * mini_batch : (k + 1) * mini_batch, :] * mask
                    )
        ...
        f = invertible_network_utils.get_decoder(
            args.x_n,
            args.z_n,
            args.seed,
            args.n_mixing_layer,
        ...
        class Constrained_DE(cooper.ConstrainedMinimizationProblem):
            ...
            def closure(self, inputs):
                ...
                ineq_defect = (
                    torch.sum(torch.abs(z_hat)) / args.batch_size / args.z_n
                    - args.sparse_level
                )

                return cooper.CMPState(
                    loss=loss + stat_loss, ineq_defect=ineq_defect, eq_defect=None
                )
        ...
        while global_step <= args.n_steps:
            if not args.evaluate:
                ...
                coop_vade.zero_grad()
                lagrangian = formulation.composite_objective(cmp_vade.closure, data)
        ...
        axes.set_title(
            rf"{mix_type} n={args.z_n} m={args.n_mixing_layer} $\delta$={args.distance}$\sigma$ $\rho$={args.mask_dense} k={args.DAG_dense}",
            fontsize=15,
        )
        ...
        if args.nn == args.z_n:
            ...
            axes.set_title(
                rf"{mix_type} n={args.z_n} nn={args.nn} m={args.n_mixing_layer} $\delta$={args.distance}$\sigma$ $\rho$={args.mask_dense} k={args.DAG_dense}",
                fontsize=15,
            )
        ...
        c_file = os.path.join(
            c_path,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}",
        )
        c_hat_file = os.path.join(
            c_hat_path,
            f"{mix_type}{args.n_mixing_layer}_d{args.distance}_n{args.z_n}_M{args.mask_dense}_G{args.DAG_dense}_rs{args.seed}",
        )
    ...
    wri = [
        ...
        args.mask_dense,
        args.DAG_dense,
        np.mean(mcc_indep_scores),
        np.std(mcc_indep_scores),
    ]
    writer.writerow(wri)
    fileobj.close()
...
def parse_args():
    ...
    parser.add_argument(
        "--distance", type=float, default=2.0, choices=[0, 1, 2, 3, 5, 10]
    )
    ...
    parser.add_argument(
        "--n-mixing-layer", type=int, default=10, choices=[1, 3, 10, 20]
    )  # larger means more complicated piecewise
    parser.add_argument("--mask-size", type=int, default=5)
    parser.add_argument("--nn", type=int)
    parser.add_argument(
        "--oracle", type=str, default="oracle", choices=["oracle", "nonoracle"]
    )
    ...
    parser.add_argument(
        "--scm-type", type=str, default="linear", choices=["linear", "nonlinear"]
    )
    parser.add_argument(
        "--noise-type", type=str, default="gauss", choices=["gauss", "exp", "gumbel"]
    )
