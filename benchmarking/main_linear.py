...
def main():
    ...
    for args.seed in args.seeds:
        ...
        class Constrained_DE(cooper.ConstrainedMinimizationProblem):
            ...
            def closure(self, inputs):
                ...
                return cooper.CMPState(
                    loss=loss, ineq_defect=ineq_defect, eq_defect=None
                )
        ...
        axes.set_title(
            rf"{mix_type} n={args.z_n} $\delta$={args.distance}$\sigma$ $\rho$={args.mask_dense} k={args.DAG_dense}",
            fontsize=15,
        )
        ...
        if args.nn == args.z_n:
            ...
            axes.set_title(
                rf"{mix_type} n={args.z_n} nn={args.nn} $\delta$={args.distance}$\sigma$ $\rho$={args.mask_dense} k={args.DAG_dense}",
                fontsize=15,
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--n-mixing-layer", type=int, default=1, choices=[1, 3, 10, 20]
    )  # larger means more complicated piecewise
    parser.add_argument(
        "--distance", type=float, default=0, choices=[0, 1, 2, 3, 5, 10]
    )
