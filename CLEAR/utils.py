...
def estimate_required_time(nb_items_in_list, current_index, time_elapsed, interval=100):
    ...
    if current_index % interval == 0 or current_index == nb_items_in_list:
        ...
        if current_index == nb_items_in_list:
            sys.stdout.write(
                f"\r{progress_bar} -- estimated required time = {time_estimation_string} -- Finished!"
            )
        else:
            sys.stdout.write(
                f"\r{progress_bar} -- estimated required time = {time_estimation_string}"
            )
...
def get_mutual_information(x, y, normalize=True):
    ...
    """
    if normalize:
        return drv.information_mutual_normalised(
            x, y, norm_factor="Y", cartesian_product=True
        )
    else:
...
def get_nb_jobs(n_jobs):
    ...
    if n_jobs != "max":
        if int(n_jobs) > mp.cpu_count():
            print(
                f'Max number of parallel jobs is "{mp.cpu_count()}" but received "{int(n_jobs)}" -- '
    ...
    for i, func_return in enumerate(pool_func(func, iterable, chunksize=chunksize)):
        ...
        if timer_verbose:
            estimate_required_time(
                nb_items_in_list=nb_items_in_list,
                current_index=i,
