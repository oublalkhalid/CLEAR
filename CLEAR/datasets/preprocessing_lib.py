...
def Upsampling_array(X, sampling: int = 1, method: str = "mean"):
    finale = np.zeros(
        (np.shape(X)[0], np.shape(X)[1], np.shape(X)[2] // (math.ceil(sampling)))
    )
    for i_eme in range(np.shape(X)[0]):
        for k in range(np.shape(X)[1]):
            if method == "mean":
                finale[i_eme, k, :] = [
                    X[i_eme, k, :][i : math.ceil(sampling) + i].mean()
                    for i in range(0, np.size(X[i_eme, k, :]), math.ceil(sampling))
                ]
            elif method == "max":
                finale[i_eme, k, :] = [
                    X[i_eme, k, :][i : math.ceil(sampling) + i].max()
                    for i in range(0, np.size(X[i_eme, k, :]), math.ceil(sampling))
                ]
            else:
...
def replace_nans_interpolation(
    aggregated: np.array = None, disaggregated: np.array = None
):
    if aggregated is not None:
        aggregated = pd.DataFrame(np.array(aggregated))
...
def normalize_sequences(
    aggregated: np.array = None,
    disaggregated: np.array = None,
    mmax: float = None,
    threshould: float = None,
):
    if aggregated is not None:
        ...
        if mmax is None:
            mmax = aggregated.max(
                axis=0
            )  # Normalisation par rapport à chaque features [activate, reactive, apparent]/ [Max_activate, Max_reactive, Max_apparent]
    ...
    if disaggregated is not None:
        ...
        disaggregated = (
            disaggregated / mmax[0]
        )  # Normalisation par rapport à l'activate uniquement comme c'est l'output
        disaggregated = disaggregated.values
...
def standardize_sequences(
    aggregated: np.array = None,
    disaggregated: np.array = None,
    mains_mean: float = None,
    mains_std: float = None,
    meter_mean: float = None,
    meter_std: float = None,
):
...
def replace_with_zero_small_values(
    aggregated: np.array, disaggregated: np.array, threshold: int
):
