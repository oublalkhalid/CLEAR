...
def _prepare_datasets(factors, codes, batch_size, nb_training, nb_eval):
    ...
    train_inputs, train_targets, line_idx_train = (
        np.zeros((nb_training, nb_codes)),
        np.zeros((nb_training,), dtype="int64"),
        0,
    )
    eval_inputs, eval_targets, line_idx_eval = (
        np.zeros((nb_eval, nb_codes)),
        np.zeros((nb_eval,), dtype="int64"),
        0,
    )
    ...
    for factor_id in range(nb_factors):
        ...
        fixed_factor_values = np.random.choice(
            available_factor_values, size=nb_factor_id_examples
        )
        ...
        for factor_value, count in fixed_factor_values.items():
            ...
            for loop_id in range(nb_loops):
                ...
                assert len(factor_value_lines_idx[:nb_examples]) == len(
                    factor_value_lines_idx[nb_examples : 2 * nb_examples]
                )
                batch_1[line_idx : line_idx + nb_examples] = factor_value_lines_idx[
                    :nb_examples
                ]
                batch_2[line_idx : line_idx + nb_examples] = factor_value_lines_idx[
                    nb_examples : 2 * nb_examples
                ]
        ...
        for idx in range(0, codes_train_1.shape[0], batch_size):
            diff = (
                codes_train_1[idx : idx + batch_size]
        ...
        for idx in range(0, codes_eval_1.shape[0], batch_size):
            diff = (
                codes_eval_1[idx : idx + batch_size]
