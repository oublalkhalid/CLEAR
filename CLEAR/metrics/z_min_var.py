...
def _prepare_datasets(
    ...
    for factor_id in range(nb_factors):
        ...
        available_factor_values = [
            value
            for value in available_factor_values
            if available_factor_values[value] > 1
        ]

        if len(available_factor_values) == 0:
            print(
                f"Error -- Factor ID: {factor_id} -- Cannot find factor values with more than 1 example -- "
        ...
        fixed_factor_values = np.random.choice(
            available_factor_values, size=factor_id_count
        )
        ...
        for factor_value, count in fixed_factor_values.items():
            ...
            for loop_id in range(nb_loops):
                ...
                batch[line_idx : line_idx + nb_examples] = factor_value_lines_idx[
                    :nb_examples
                ]
            ...
            for id in range(nb_factors):
                if id == factor_id:
                    assert (
                        np.min(factors_batch[:, id])
                        == np.max(factors_batch[:, id])
                        == factor_value
                    )
                else:
                    if (
                        np.min(factors_batch[:, id]) == np.max(factors_batch[:, id])
                        and verbose
                    ):
                        print(
                            f"Warning -- Factor ID: {id} -- factor values are equal "
