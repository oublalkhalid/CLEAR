...
def z_max_var(
    ...
    train_set, eval_set = _prepare_datasets(
        factors=factors,
        codes=codes,
    ...
    train_accuracy = (
        np.sum(training_votes[latent_idx, classifier]) * 1.0 / np.sum(training_votes)
    )

    # compute evaluation accuracy
    eval_votes = _compute_votes(
        inputs=eval_set[:, 0],
        targets=eval_set[:, 1],
        nb_codes=nb_codes,
        nb_factors=nb_factors,
    ...
    eval_accuracy = (
        np.sum(eval_votes[latent_idx, classifier]) * 1.0 / np.sum(eval_votes)
    )
...
def _get_factor_values_combinations(factors, factor_id):
    ...
    if len(combins) == 0:
        print(
            f"Error -- Factor ID: {factor_id} -- Cannot find factor values combinations with more than 1 example -- "
...
def _get_discrete_factor_values_lines_idx(factors, factor_id, factor_values):
    ...
    for column_idx, factor_value in enumerate(factor_values):
        if lines_idx == []:
            ...
            lines_idx = set(lines_idx) & set(
                np.where(factors_new[:, column_idx] == factor_value)[0].tolist()
            )
...
def _prepare_datasets(
    factors, codes, batch_size, nb_training, nb_eval, variances, active_dims, verbose
):
    ...
    for factor_id in range(nb_factors):
        ...
        if factor_values_combinations is NaN:
            return (
                NaN,
                NaN,
            )  # score cannot be computed correctly because of discretization

        # choose fixed random values for other factor IDs, for each batch of examples
        fixed_factor_vals = [
            random.choice(factor_values_combinations) for _ in range(factor_id_count)
        ]
        ...
        for factor_values, count in fixed_factor_values.items():
            ...
            factor_values_lines_idx = _get_discrete_factor_values_lines_idx(
                factors, factor_id, factor_values
            )
            ...
            for loop_id in range(nb_loops):
                ...
                batch[line_idx : line_idx + nb_examples] = factor_values_lines_idx[
                    :nb_examples
                ]
            ...
            for id, val in zip(
                [fact for fact in range(nb_factors) if fact != factor_id], factor_values
            ):
                assert (
                    np.min(factors_batch[:, id]) == np.max(factors_batch[:, id]) == val
                )

            if (
                np.min(factors_batch[:, factor_id])
                == np.max(factors_batch[:, factor_id])
                and verbose
            ):
                print(
                    f"Warning -- Factor ID: {factor_id} -- factor values are equal "
            ...
            for idx in range(0, codes_train.shape[0], batch_size):
                argmax = _generate_sample(
                    codes_train[idx : idx + batch_size],
                    axis=0,
                    variances=variances,
                    active_dims=active_dims,
                )
