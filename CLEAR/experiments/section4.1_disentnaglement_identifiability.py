...
from utils import (
    ...
    get_artificial_factors_dataset,
...
"""
    Experiment of section 4.1: "Disentnaglement with identifiability"
"""
...
def get_factors_codes_dataset(noise_level):
    ...
    factors = get_artificial_factors_dataset(
...
def run_noise_experiment(sub_parser_args):
    ...
    seeds = get_experiment_seeds(
        nb_representations=NB_RANDOM_REPRESENTATIONS, nb_runs=NB_RUNS
    )
    ...
    for metric in metrics:
        ...
        scores_array = np.zeros(
            (len(noise_array), NB_RANDOM_REPRESENTATIONS, NB_RUNS)
        ).squeeze()
        ...
        if "DCI" in metric and "MIG" not in metric:
            ...
            metric_scores = {
                f"{metric} Mod": scores_array.copy(),
        ...
        for line_idx, noise_level in enumerate(NOISE_LEVEL):
            ...
            factors_codes_dataset = partial(
                get_factors_codes_dataset, noise_level=noise_level
            )
            scores = launch_multi_process(
                iterable=seeds,
                func=get_score,
                n_jobs=NB_JOBS,
                timer_verbose=False,
                metric=metric_func,
                factors_codes_dataset=factors_codes_dataset,
            )
            ...
            estimate_required_time(
                nb_items_in_list=len(seeds) * len(NOISE_LEVEL),
...
NB_EXAMPLES = 10
NB_FACTORS = 8
DISTRIBUTION = [np.random.uniform, {"low": 0.0, "high": 1.0}]
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(PROJECT_ROOT, "results", "section5.2_noise")

    # create parser
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers()

    # parser for the "run" command -- run noise experiment
